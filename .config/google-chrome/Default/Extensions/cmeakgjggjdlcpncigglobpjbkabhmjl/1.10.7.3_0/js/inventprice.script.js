...
var g_isLoadedFloatValue = false;
var lastSort = null;
var userInventory = [];
...
var getLowestPriceHandler = function (gitem, callback) {
    ...
    //$J('.dd_price').html("Loading...");
    var isGenuine = false;
    var itemLink = window.location.protocol + "//steamcommunity.com/market/priceoverview/?appid=" + sItem.appid + "&country=" + g_strCountryCode + "&currency=" + currencyId + "&market_hash_name=" + encodeURIComponent(sItem.description.market_hash_name);
    var itemLinkW = window.location.protocol + "//steamcommunity.com/market/priceoverview/?appid=" + sItem.appid + "&country=" + g_strCountryCode + "&currency=" + sellcurrencyId + "&market_hash_name=" + encodeURIComponent(sItem.description.market_hash_name);
    ...
    if (!numberOwnedDesc.length) {
        sItem.description.descriptions.unshift({
            iscount: true,
            type: 'html',
            value: 'Number owned: <i style="color: rgb(153, 204, 255); font-size: 16px">' + sItem.description.use_count + '</i>' + (g_bViewingOwnProfile ? ' (<a href="javascript:selectSimilar(' + sItem.classid + ')">Select all</a>)' : '')
        });
    }
...
var ShowQueue = function (goo) {
    ...
    selectedItems.each(function () {
        ...
        if (window.extmasslisting && rgItem.description.extprice) {
            var extpricetag = $J('<div class="p-price"></div>');
            extpricetag.prop('title', 'scmspy');
            // extpricetag.text(ExchangeRates.Format(rgItem.description.extprice));
            extpricetag.text(v_currencyformat(rgItem.description.extprice * 100, GetCurrencyCode(typeof (g_rgWalletInfo) != 'undefined' ? g_rgWalletInfo.wallet_currency : 1)));
...
var ModifyMarketActions = function () {
    ...
    PopulateMarketActions = function (elActions, item) {
        ...
        if (typeof (g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile) {
            var strMarketName = typeof item.description.market_hash_name != 'undefined' ? item.description.market_hash_name : item.description.market_name || item.description.name;

            var elPriceInfo = new Element('div');
            var elPriceInfoHeader = new Element('div', {'style': 'height: 24px;'});
            var marketListingsURL = window.location.protocol + '//steamcommunity.com/market/listings/' + item.appid + '/' + encodeURIComponent(strMarketName);
            var elMarketLink = new Element('a', {
                'href': marketListingsURL
            });
            ...
            $J(elSellButton).css({'marginRight': '12px'});
              ...
              $J.ajax({
                ...
                $J.ajax({
                  ...
                  if (data.success && data.highest_buy_order) {
                    ...
                    var instantSellBt = CreateMarketActionButton('green', '#',  SIHLang.instantsell.replace('$1', sellingPrice));
                    $J(instantSellBt).css({'marginTop': '12px'});
...
var GetCSGOItems = function () {
    var params = {
        type: 'GetPlayerItems',
        steamid: g_ActiveUser.strSteamId,
        appid: 730
    };
    chrome.runtime.sendMessage(SIHID, params, function (resp) {
      if (!apiItems[params.steamid]) {
          apiItems[params.steamid.toString()] = {};
      }

      if (resp && resp.success) {
          apiItems[params.steamid][730] = resp.data;
          $J.each(apiItems[params.steamid][730], function (i, o) {
              var elIt = $J('div.item[id=730_2_' + o.id + ']');
              if (o.float_value) {
                  var dPstr = '';
                  if (o.doppler_phase) {
                      dPstr = dopplerPhaseName[o.doppler_phase] || '';
                  }
                  elIt.each(function (idx, elem) {
                      if (!elem.rgItem.float_value) {
                          elem.rgItem.float_value = o.float_value;
                          elem.rgItem.dopplerPhase = dPstr;
                      }
                  });
                  if (window.show_float_value) {
                    elIt.append('<div class="float-value">Float: <strong>' + o.float_value.toFixed(4) + '</strong></div>');
                  }
              }
          });
          g_isLoadedFloatValue = true;
      }
    });
    // $J.ajax({
    //     url: '//api.steampowered.com/IEconItems_730/GetPlayerItems/v0001/',
    ...
    //                     elIt.each(function (idx, elem) {
    //                         if (!elem.rgItem.float_value) {
    //                             elem.rgItem.float_value = floatValue;
    //                             elem.rgItem.dopplerPhase = dPstr;
    //                             // if (elem.rgItem.description.descriptions) {
    //                             //     // elem.rgItem.description.descriptions = elem.rgItem.description.descriptions.filter(x => !x.floatvalue);
    //                             //     elem.rgItem.description.descriptions.unshift({
    //                             //         type: "html",
    ...
    //                     });
    //                     if (window.show_float_value) {
    //                       elIt.append('<div class="float-value">Float: <strong>' + floatValue.toFixed(4) + '</strong></div>');
    //                     }
    //                 }
    //             });
    //             g_isLoadedFloatValue = true;
...
var UpdateSellItem = function (item) {
    if ($J('#ck_autoadjust').is(':checked')) {
        if (item.description.lowestPriceW === undefined) {
...
var AddDialogHTML = function () {
    var dialog = '<div id="market_buyset_dialog" class="market_modal_dialog" style="display: none;">' +
        ...
        '<a id="market_buyset_dialog_reload" href="#" class="btn_green_white_innerfade btn_small_wide"><span data-lang="tradingcards.reload">Reload list</span></a>' +
        '<div>&nbsp;<br /><br /></div>' +
        '</div>' +
        '</div>' +
        '</div>' +
...
var addSortItemsButton = function () {
  ...
  if (g_ActiveInventory.appid != 730) $J('#Lnk_SortItems').hide();
};
...
setTimeout(function () {
    ...
    $J('body').on('click', '.item', function (e) {
        ...
        if (g_ActiveInventory.appid == 730 && !g_isLoadedFloatValue) {
          numberOfRetries = 0;
          GetCSGOItems();
        }
        getLowestPriceHandler();
    ...
    if (window.inventoryprice) {
        ...
        $J('.games_list_tab').on('click', function () {
            $J('#total-price').text('Total price');
        });
        ...
        $J('body').on('click', '.lnk_inventory_price', function () {
            ...
            if (g_ActiveInventory.appid == 730) {
              chrome.runtime.sendMessage(SIHID, {type: 'GetInventoryValue', data: g_ActiveUser.strSteamId}, function (resp) {
                if (resp.success) {
                  // var localTotal = resp.value * ExchangeRates.GetCurrentRate();
                  // $total.text(v_currencyformat(localTotal * 100, GetCurrencyCode(g_rgWalletInfo.wallet_currency)));
    ...
    $J('.games_list_tab').click(function () {
      if ($J(this).attr('id') == 'inventory_link_730') {
        setTimeout(function () {
          ...
          if (lastSort !== null) SortItem(lastSort);
        }, 300);
      } else {
    ...
    function getGameInfo() {
        ...
        $J.ajax({
            ...
            if (res.success) {
                ...
                for (var i in items) {
                    if (!items[i].tradable) {
                        var ownerDescriptions = items[i].owner_descriptions;
                        if (!ownerDescriptions)
                            break;

                        var size = items[i].owner_descriptions.length;
                        for (var j = 0; j < size; ++j) {
                            var time = findDate(ownerDescriptions[j].value);
                            if (time) {
                                nonTradableCounter++;
            ...
            if (++counter === dataSize) {
                if (nonTradableCounter) {
                    tradableDates.sort();
                    $J('#tradable-msg-holder').html(makeItemsInfoMessage(nonTradableCounter, tradableDates.shift(), tradableDates.pop()));
...
function makeItemsInfoMessage(count, timeStart, timeEnd) {
    return '<span class="msg">' + count + ' ' +  SIHLang.nontradable.counter + '.' +
        '<br/>' + SIHLang.nontradable.startdate + ': ' + formatDate(timeStart) + '.' +
        '<br/>' + SIHLang.nontradable.lastdate + ': ' + formatDate(timeEnd) + '.' +
