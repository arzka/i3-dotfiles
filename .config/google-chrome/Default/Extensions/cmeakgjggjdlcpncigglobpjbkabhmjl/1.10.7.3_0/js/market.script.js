      ...
      if ($J(this).hasClass('market_listing_buyorder_qty')) {
        ...
        $J(this).click(function () {
          ...
          $rowsli.sort(function (a, b) {
            ...
            var aprice = $J(a).find('[title="This is the price the buyer pays."]').text() || /(\d+[\.|\,]\d+)/.exec($J(a).find('.market_listing_price').text())[0];
            var bprice = $J(b).find('[title="This is the price the buyer pays."]').text() || /(\d+[\.|\,]\d+)/.exec($J(b).find('.market_listing_price').text())[0];
            var an = parseFloat(getNumber(aprice)),
            bn = parseFloat(getNumber(bprice));
            if (an == bn) {
              an = $J(a).find('.market_listing_item_name_link').text();
              bn = $J(b).find('.market_listing_item_name_link').text();
            }
    ...
    if (typeof (window.totalrow) === 'undefined' || window.totalrow) {
      $J('#myListings .market_content_block.my_listing_section.market_home_listing_table').each(function (idx, listingsTable) {
        ...
        $listingsTable.find('.market_listing_price').children('span').each(function (rowId, price) {
            ...
            var pays = parseFloat(getNumber(buyerPrice));
            if (!isNaN(pays)) totalSelling += pays * 100;

            var yourPrice = isBuyOrder ? '0.00' : $price.find('span:last').text();
            var recieve = parseFloat(getNumber(yourPrice));
            if (!isNaN(recieve)) totalRecieve += recieve * 100;
        });
...
var GetActualItemPrice = function (self) {
    ...
    $J.each(assetList, function (i, row) {
        if (!row.actions[0]) return true;

        var key = row.actions[0].link.split('A%')[0].split('%20M')[1];
        if (key == $itemNameElement.attr('id').split('_')[2]) {
            hashName = row.market_name;
            return false;
        }
    });

    if (!hashName) hashName = $itemNameElement.text();
...
var addHideListingButton = function () {
  var btnPanel = [
    '<a href="javascript:void(0)" id="btnHideListing" data-hidden="false" class="item_market_action_button item_market_action_button_green" style="float: right; right: 15px; position: relative;">',
      '<span class="item_market_action_button_contents" style="min-width: 80px;">Hide listings</span>',
      '<span class="item_market_action_button_preload"></span>',
    '</a>'].join('');
  ...
  $J('#btnHideListing').click(function () {
    ...
    if (isHidden) {
      ...
      $elem.find('.item_market_action_button_contents').text('Hide listings');
      ...
      $elem.find('.item_market_action_button_contents').text('Show listings');
...
$J(function () {
                ...
                g_oMyListings = new CAjaxPagingControls(
                  {
                    ...
                    class_prefix: 'market'
                  }, 'http://steamcommunity.com/market/mylistings/'
                );
    ...
    $J('#mainContents')
        .on('click', '.market_paging_pagelink', function () {
