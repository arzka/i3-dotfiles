...
chrome.alarms.onAlarm.addListener(function (alarm) {
    if (alarm.name === 'checkUser') {
        checkUser();
        return;
    }

    if (alarm.name == 'sendStat') {
        // sendStat(chrome.runtime.getManifest().version);
        return;
    }
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    if (request.type == 'adstat') {
        return saveStat(request.data, request.action);
    }
});
...
chrome.runtime.onMessageExternal.addListener(
    function (request, sender, sendResponse) {
        if (request.type == 'CSGOFAST') {
            ...
            return true;
        } else if (request.type === 'CSGOBACKPACK') {
            getCSGOBACKPACKPrices(sendResponse, request.data);
            return true;
        } else if (request.type === 'OPSKINS') {
            getOPSkinsPrices(sendResponse, request.appid);
            return true;
        } else if (request.type === 'GetInventoryValue') {
          GetInventoryValue(request.data, sendResponse);
          return true;
        } else if (request.type === 'GetPlayerItems') {
