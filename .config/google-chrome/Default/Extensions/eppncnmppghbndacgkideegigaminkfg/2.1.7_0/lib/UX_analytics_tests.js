...
(function() {
	...
	let root = this;
	let library = {};
	let libraryExportName = 'UX_stats';
	...
	//   }
	//   exports[libraryExportName] = library;
	// } else {
	  root[libraryExportName] = library;
	// }
	...
	library.init = function(trackingID,extensionID) {
		// Production trackingID: UA-84668833-3
		// Development trackingID: UA-84668833-2
    ...
    library.sm = {
            install(version) {
                if(!extension.id) {
                    console.log('Incorrect init: extension.id missing');
                    return
                }
                ga('send', {
                  hitType: 'event',
                  eventCategory: extension.id,
	...
	library.extension = {
			...
			update(info) {
				...
				ga('send', {
				  ...
				  eventCategory: extension.id,
				  eventAction: 'update',
				  eventLabel: `Extension ${extension.id}`
				});
			}
	}
}.call(this));

UX_stats.init('UA-84668833-2',chrome.runtime.id);

let UI_test = function () {
	chrome.runtime.onInstalled.addListener(function(details){
    if(details.reason == "install"){
        setTimeout(function () {
            console.log('Installs event');
            UX_stats.extension.install();
        },1000*5);
    }else if(details.reason == "update"){
        setTimeout(function () {
            console.log('Updates event');
            UX_stats.extension.update();
        },1000*5);
    }
});
_.loaded()
